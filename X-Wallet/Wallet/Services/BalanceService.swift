...
public class BalanceService: BalancerUpdater {
    ...
    func getCoinBalance(chain: Chain, address: String) async throws -> AssetBalance {
        return try await chainServiceFactory.service(for: chain).coinBalance(for: address)
    }
    ...
    func createBalanceUpdate(assets: [Asset], balances: [AssetBalance], prices: [AssetPrice]) -> [UpdateBalance] {
        ...
        return balances.compactMap { (balance: AssetBalance) in
            guard
                ...
                let total = try? formatter.double(
                    from: balance.balance.total(asset.chain.includeStakedBalance),
