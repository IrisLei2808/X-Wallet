...
struct SwapView: View {
    ...
    @State var sendNetwork: AddNetwork =  AddNetwork(
        id: "fd25a1de-427f-11ee-be56-0242ac120002",
        name: "Ethereum",
        imageName: "ethereum",
        amount: 123456,
        added: false,
    ...
    @State var receiveNetwork: AddNetwork =  AddNetwork(
        id: "fd25a0e4-427f-11ee-be56-0242ac120002",
        name: "Bitcoin",
        imageName: "bitcoin",
        amount: 123456,
        added: true,
            ...
            ScrollView {
                VStack {
                    ZStack(alignment: .center) {
                        VStack {
                            SwapCardView(
                                title: data.strings.from,
                                tokenName: sendNetwork.code,
                                tokenImageStr: sendNetwork.imageName,
                                currentTransactionalTokenAmount: $sendingTransactionalTokenAmount, amountInUSD:   (sendingTransactionalTokenAmount.fractionNumberDouble * 1.5 ).dollar,
                                availableTokenAmt: sendNetwork.amount.fractionNumber, curentTransactionalAmountPlaceHolder: data.strings.enterAmount,
                                symbol: sendNetwork.code) {
                                selectSendNetwork = true
                                showSelectToken = true
                            }
                            SwapCardView(
                                title: data.strings.receive,
                                tokenName: receiveNetwork.code,
                                tokenImageStr: receiveNetwork.imageName,
                                currentTransactionalTokenAmount: $receivingTransactionalTokenAmount, amountInUSD: (
                                    sendingTransactionalTokenAmount.fractionNumberDouble * 1.5 - 0.002
                                ).zeroIfNegative.dollar, curentTransactionalAmountPlaceHolder: "",disableUserInteraction: true, symbol: receiveNetwork.code) {
                    ...
                    ForEach(Array(info.enumerated()), id: \.0) { (index, item) in
                        VStack(spacing: 16) {
                            HStack {
                                ...
                                CustomText(
                                    name: item.key == data.strings.exchangeRate ?
                                    "1\(sendNetwork.code) = 1.5\(receiveNetwork.code)" :
                                        item.value,
                                    alignment: .trailing,
